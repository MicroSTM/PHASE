...
pos_by_rm_dict = {'rm0pos':[0, 1, 10, 14], 'rm1pos':[4, 5, 12, 16], 'rm2pos':[6, 7, 13, 17], 'rm3pos':[2, 3, 11, 15]}
landmarks = [0, 1, 2, 3]
items = [0, 1]
agents = [0, 1]
...
for env in envs:
    ...
    if goal_pre == 'LMO':
        ...
        #lm
        goal_var2_hindered = random.choice(list(set(landmarks)-set([goal_item_rm]))) #lm != item rm
        goal_var2_hinder = random.choice(list(set(landmarks)-set([goal_item_rm,goal_var2_hindered])))
        hindered_rm = random.choice(landmarks)
        if get_rm_path(goal_item_rm, goal_var2_hindered) in free_rms and\
            get_rm_path(goal_item_rm, goal_var2_hinder) in free_rms:
            goal_item_size = random.choice(all_sizes)
        else:
            goal_item_size = random.choice(small_sizes)
        if get_rm_path(hinder_rm, goal_item_rm) in free_rms and \
            get_rm_path(goal_item_rm, goal_var2_hinder) in free_rms:
            hinder_size = random.choice(all_sizes)
        else:
            hinder_size = random.choice(small_sizes)
        if get_rm_path(hindered_rm, goal_item_rm) in free_rms and \
            get_rm_path(goal_item_rm, goal_var2_hindered) in free_rms:
    ...
    if goal_pre == 'LMA':
        ...
        hindered_rm = random.choice(list(set(landmarks)-set([goal_var2]))) #agent rm != lm rm
        goal_item_size = random.choice(all_sizes)
    ...
    if goal_pre == 'TE':
        ...
        goal_var1 = hindered_id
        #item
        goal_var2 = goal_item_id + len(items) #entity id
        hindered_rm = random.choice(list(set(landmarks)-set([goal_item_rm]))) #agent rm != item rm
        goal_item_size = random.choice(all_sizes)
    ...
    alphas['agent'+str(hinder_id)].append(0)
    #depends on rooms
    ...
    #random
    angles['agent'+str(hindered_id)].append(round(math.radians(np.random.uniform(-360,360)),2))
    angles['agent'+str(hinder_id)].append(round(math.radians(np.random.uniform(-360,360)),2))
    #same goal (except hinder)
    goal_pres.append(goal_pre)
    goal_ag_it.append(goal_var1)
    goal_lm_it['agent'+str(hindered_id)].append(goal_var2_hindered)
    goal_lm_it['agent'+str(hinder_id)].append(goal_var2_hinder)
    goals_end.append(1)
    #depends on setup
    init_positions['agent'+str(hindered_id)].append(random.choice(pos_by_rm_dict['rm'+str(hindered_rm)+'pos']))
    init_positions['agent'+str(hinder_id)].append(random.choice(pos_by_rm_dict['rm'+str(hinder_rm)+'pos']))
...
filename = 'compete.txt'
text_file = open(filename, "w")
text_file.write(" ".join(map(str,envs)))
print(" ".join(map(str,envs)))
for param_holder in [alphas, sizes, strengths, angles, goal_pres, goal_ag_it, goal_lm_it, \
                   goals_end, init_positions]:
