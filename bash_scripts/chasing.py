...
for env in envs:
    ...
    if goal_pre == "TE":
        # chasing agent
        goal_var1 = hinder_id
        # chased (negative goal)
        goal_var2 = hindered_id
        hindered_rm = random.choice(
            list(set(landmarks) - set([hinder_rm]))
        )  # agent rm != item rm
        goal_item_size = random.choice(all_sizes)
        if get_rm_path(hindered_rm, hinder_rm) in free_rms:
            hinder_size = random.choice(all_sizes)
            hindered_size = random.choice(all_sizes)
        else:
            hinder_size = random.choice(small_sizes)
    ...
    alphas["agent" + str(hindered_id)].append(-10)
    # depends on rooms
